...
window.addEventListener("DOMContentLoaded", () => {
  document.getElementById("cityInput").value = "Haldia"; // default city
...
function fetchWeather() {
  ...
  fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`)
    ...
    .then(data => {
      ...
      timeData.innerHTML = `<h3>🕓 Time: ${timeNow}</h3>`;
      ...
      const mainSection1 = document.querySelector(".tempDetails, .time");

      if (hour >= 18 || hour < 6) {
        mainSection1.style.backgroundImage = "url('images/night.jpg')";
        mainSection1.style.backgroundSize = "cover";
        mainSection1.style.backgroundPosition = "center";
      } else {
        mainSection1.style.backgroundImage = "url('images/day.jpg')";
        mainSection1.style.backgroundSize = "cover";
        mainSection1.style.backgroundPosition = "center";
      }
      ...
      wCondition.innerHTML = `
        ...
        <p>${weather.main} - ${data.weather.description}</p>
      `;
